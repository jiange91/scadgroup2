...
def main(params, action):
    ...
    localPool = {}
    selfPool = ObjPool(name='tagNameOnFrames-worker-1-mem', memSize=
        536870912, pageSize=16384)
    selfPool.registerTrans(actionLib=action)
    uploads = {'tagNameOnFrames-exit': [('face_namesPF', False, True)]}
    pullR = {'target_encodings', 'target_name', 'face_encodingsPF'}
    workloads = context_dict['workloads']
    stkTargets = context_dict['tagNameOnFrames-worker-1']
    ...
    inMap = {'target_encodings', 'target_name', 'face_encodingsPF'}
    inDict = {}
    for inName in inMap:
        inDict[inName] = localPool[inName]
    outDict = tagTargetFrames(**inDict)
    localPool.update(outDict)
    for vname, obj in outDict.items():
        selfPool.upload_remote(obj, vname)
    context_dict = {}
    context_dict['tagNameOnFrames-worker-1-pool'] = selfPool
